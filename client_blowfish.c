...
this version using blowfish on payloads

to run 
# gcc -o client_blowfish.c client_blowfish -lssl -lcrypto
#./client IP_SERVER 
...
// start blowfish key
 ...
 // fprintf(stdout,"fate  : %s\n",argv[1]);    
  ...
  {     
   ...
   chomp(input);
   BF_ecb_encrypt((unsigned char *)input, cripto_out, key, BF_ENCRYPT);
   fazerpacote(destino, PORT,(char *)cripto_out);
